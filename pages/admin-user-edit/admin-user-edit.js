...
Page({
    ...
    if (id) {
      ...
      wx.cloud.callFunction({ name: DBQUERY_FUNCTION, data: { collection: 'users', docId: id } })
        .then((res) => {
          ...
          this.setData({ username: u.username || '', roleIndex: ri >= 0 ? ri : 2, statusIndex: si >= 0 ? si : 0 })
        })
    } else {
      try { wx.setNavigationBarTitle({ title: '新增人员' }) } catch (e) {}
    }
  },
  onUserInput(e) { const v = String(e.detail.value || '').slice(0,30); this.setData({ username: v }) },
  ...
  submit() {
    ...
    if (this.data.docId) {
      const data = { username: name, role, status, updatedAt: nowStr }
      ...
      const data = { id: `user_${Date.now()}`, username: name, role, status, createdAt: nowStr, updatedAt: nowStr }
